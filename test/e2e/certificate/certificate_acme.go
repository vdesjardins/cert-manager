...
var _ = framework.CertManagerDescribe("ACME Certificate (HTTP01)", func() {
	...
	cases := []struct {
	...
	}{
		{
			...
			label:            "should obtain a valid certificate with duration of 90 days from ACME server configured with a TTL of 90 days when asking for a duration of 365 days",
		...
		},
		{
			...
			expectedDuration: time.Hour * 24 * 90,
			label:            "should obtain a valid certificate with a warning event when renewBefore is bigger than the duration",
